 ...
 */
export async function getWebCodeAndSmsStatus(datodberu, datumnarod, dbConfig) {
    const dbType = dbConfig.type || 'sybase';

    if (dbType !== 'sybase') {
        console.warn('Web code and SMS status search is only supported for Sybase DB.');
        return null;
    }
...
/**
 * @description Отримує статистику коштів по направляючих лікарях за період (тільки для Sybase).
 * @param {object} params - Параметри запиту.
 ...
 * @returns {Promise<Array<object>|undefined>}
 */
export async function getReferringDoctorCashStats({ dateFrom, dateTo }, dbConfig) {
    const dbType = dbConfig.type || 'sybase';

    if (dbType !== 'sybase') {
        console.warn('Referring doctor cash stats report is only supported for Sybase DB.');
        return [];
    }

    const query = `
        SELECT 
            ko.nazovodd AS "Направляючий",
            COUNT(DISTINCT zp.evidcis) AS "Кількість",
            SUM(vyk.cena) AS "Сума"
        FROM 
            ziad_okb_pom zp
        JOIN 
            c_zak_okb ko ON zp.kododdokb = ko.kododdokb
        JOIN 
            c_okb_vykon vyk ON zp.kodvys = vyk.kodvys
        WHERE 
            zp.datodberu BETWEEN '${dateFrom}' AND '${dateTo}'
            AND zp.kodlab = '00001'
