...
export default function createPDF(secData, data, backgroundImagePath, fontPath, backgroundTrigger=false) {
    ...
    return new Promise((resolve, reject) => {
        ...
        rowsArray.forEach((row, rowIndex) => {
            ...
            if (!panel) { }
                ...
                const fontSize = 8.5;
                const textWidth = doc.widthOfString(panel, fontSize);
                ...
                doc.font(fontPath).fontSize(9).opacity(1).fillColor('black');
                doc.text(panel, centerX + 30, currentY + 3, { fontSize: fontSize });
            ...
            for (let index = 0; index < row.length; index++) {
                ...
                if (index === 0) {
                    if (([":", "а саме:"].includes(row[4]) && !excludedPhrases.some(phrase => row[0].includes(phrase)))) {  // [':','а саме:'].includes(row[4])        //  row[4]=== ':'
                        const pageWidth = doc.page.width;
                        // Розмір шрифту
                        const fontSize = 8.5;
     ...
     if (backgroundTrigger){
        ...
        writableStream.on('finish', () => {
            try {
                // Створюємо один великий буфер з усіх частин
                const pdfBuffer = Buffer.concat(buffers);
                
                // Повертаємо (resolve) Promise з готовим буфером
                resolve(pdfBuffer);

            } catch (error) {
                // Якщо сталася помилка при конкатенації
                reject(error);
            }
        });
    });
