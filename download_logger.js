...
import { Mutex } from 'async-mutex'; // <-- ІМПОРТУЄМО MUTEX
...
const mutex = new Mutex(); // <-- СТВОРЮЄМО ЕКЗЕМПЛЯР MUTEX
...
export async function logDownload(branchDepId, accessKey) {
    // Захоплюємо м'ютекс. Наступні виклики чекатимуть, поки поточний не завершиться.
    ...
    try {
        ...
        release(); // <-- ВАЖЛИВО: звільняємо м'ютекс, щоб інші могли працювати з файлом.
    }
}